...
```{r}
joined_candy %>% 
  select(age_clean, age_demog_q) %>% 
  filter(str_detect(age_demog_q,"\\d", negate = TRUE)) %>% 
...
```{r}
#cleaning age data
# this is not quite right but leave for now!
...
testdata <- joined_candy %>% 
  ...
  pull()
#ind <-  which(str_detect(testdata,"'s"))
ind <-  which(str_detect(testdata,"\\d(ish|s|'|approx|>)"))
#ind <-  which(str_detect(testdata,"\\d{1,2}"))
str_extract(unique(testdata[ind]),"\\d{1,2}")


```

```{r}
#check what is in 'other'
joined_candy %>% 
  select(age_clean,age_demog_q) %>% 
  filter(age_clean == "other") %>% 
  distinct(age_demog_q)

#quick summary to sense check results
joined_candy %>% 
  select(age_clean) %>% 
  summarise(min = min(age_clean, na.rm=TRUE), max = max(age_clean, na.rm = TRUE), mean = mean(age_clean, na.rm = TRUE))

```
```{r}
#check upper and lower values
joined_candy %>% 
   select(age_clean,age_demog_q) %>% 
   arrange(age_clean) %>%
   distinct(age_clean,age_demog_q) %>% 
   head(30)
```
...
```{r}
joined_candy %>% 
  group_by(gender_demog_q) %>% 
  distinct(gender_demog_q)
```
```{r}
joined_candy %>% 
  group_by(tot_demog_q) %>% 
  distinct(tot_demog_q)
```
